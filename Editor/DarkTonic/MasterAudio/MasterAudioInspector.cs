...
public class MasterAudioInspector : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        if (_sounds.showMusicDucking) {
            ...
            var newMult = EditorGUILayout.Slider("Default Ducked Vol Multiplier", _sounds.defaultDuckedVolumeMultiplier, 0f, 1f);
            if (newMult != _sounds.defaultDuckedVolumeMultiplier) {
                AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Default Ducked Vol Multiplier");
                _sounds.defaultDuckedVolumeMultiplier = newMult;
            }
            ...
            newDefault = EditorGUILayout.Slider("Default Unduck Time", _sounds.defaultUnduckTime, 0f, 5f);
            ...
            if (GUILayout.Button(new GUIContent("Add Duck Group"), EditorStyles.toolbarButton, GUILayout.Width(100))) {
                ...
                _sounds.musicDuckingSounds.Add(new DuckGroupInfo() {
                    ...
                    duckedVolumeMultiplier = _sounds.defaultDuckedVolumeMultiplier,
            ...
            if (_sounds.musicDuckingSounds.Count == 0) {
                ...
                if (_sounds.musicDuckingSounds.Count > 0) {
                    ...
                    GUILayout.FlexibleSpace();
                    GUILayout.Label(new GUIContent("Vol. Multiply", "Amount to duck the music volume."), EditorStyles.boldLabel);
                    GUILayout.Space(9);
                ...
                for (var i = 0; i < _sounds.musicDuckingSounds.Count; i++) {
                    ...
                    GUI.contentColor = DTGUIHelper.BrightButtonColor;
                    GUILayout.TextField(duckSound.duckedVolumeMultiplier.ToString("N2"), 20, EditorStyles.miniLabel);

                    var newDuckMult = GUILayout.HorizontalSlider(duckSound.duckedVolumeMultiplier, 0f, 1f, GUILayout.Width(60));
                    if (newDuckMult != duckSound.duckedVolumeMultiplier) {
                        AudioUndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _sounds, "change Ducked Vol Multiplier");
                        duckSound.duckedVolumeMultiplier = newDuckMult;
                    }
                    GUI.contentColor = Color.white;
