...
public class AudioCalcEventInspector : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        var sounds = (EventCalcSounds)target;
        ...
        if (aud == null || aud.clip == null) {
            DTGUIHelper.ShowRedError("Audio Source Ended Sound - needs AudioSource component and an Audio Clip assigned.");
            ...
            if (sounds.useAudioSourceEndedSound && !sounds.disableSounds) {
                ...
                if (_maInScene) {
                    var existingIndex = _groupNames.IndexOf(sounds.audioSourceEndedSound.soundType);
                    ...
                    } else if (existingIndex == -1 && sounds.audioSourceEndedSound.soundType == MasterAudio.NoGroupName) {
                        groupIndex = EditorGUILayout.Popup("Sound Group", existingIndex, _groupNames.ToArray());
                    } else { // non-match
                        noMatch = true;
                    ...
                    if (groupIndex.HasValue)
                    {
                        ...
                        {
                            case -1:
                                sounds.audioSourceEndedSound.soundType = MasterAudio.NoGroupName;
                                break;
                            default:
                                sounds.audioSourceEndedSound.soundType = _groupNames[groupIndex.Value];
                                break;
                        }
                    }
                } else {
				...
				var newVolume = DTGUIHelper.DisplayVolumeField(sounds.audioSourceEndedSound.volume, DTGUIHelper.VolumeFieldType.None, false, 0f, true);
                ...
                if (sounds.audioSourceEndedSound.useFixedPitch) {
                    ...
                    var newFixedPitch = DTGUIHelper.DisplayPitchField(sounds.audioSourceEndedSound.pitch);
