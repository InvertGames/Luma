...
public class PlaylistController : MonoBehaviour {
    ...
    void Update() {
        ...
        if (!_activeAudio.loop && _activeAudio.clip != null) {
			...
			if (!_activeAudio.isPlaying) {
				...
				var clipFadeStartTime = Time.deltaTime * EventCalcSounds.FramesEarlyToTrigger * _activeAudio.pitch;
	...
	private void RemoveScheduledClip() {
		...
		private void ScheduleClipPlay(double scheduledPlayTime) {
            _activeAudio.PlayScheduled(scheduledPlayTime);			

			RemoveScheduledClip();

            _scheduledSongsByAudioSource.Add(_activeAudio, scheduledPlayTime);
