	...
	public class UIRectangularRaycastFilter : MonoBehaviour, ICanvasRaycastFilter
	{
		...
		public Rect scaledRect
		{
			get
			{
				...
				return new Rect(
					(this.offset.x + (rt.rect.x + ((rt.rect.width - (rt.rect.width * this.m_ScaleX)) / 2f))), 
					(this.offset.y + (rt.rect.y + ((rt.rect.height - (rt.rect.height * this.m_ScaleY)) / 2f))), 
					(rt.rect.width * this.m_ScaleX), 
					(rt.rect.height * this.m_ScaleY)
				);
			}
		}
