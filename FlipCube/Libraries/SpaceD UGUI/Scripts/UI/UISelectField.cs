...
namespace UnityEngine.UI
{
	...
	public class UISelectField : Toggle {
		...
		public new AnimationTriggersExtended animationTriggers;
		...
		/// <summary>
		/// Sets the starting option (Editor Only).
		/// </summary>
		/// <param name="optionValue">Option value.</param>
		public void SetStartingOption(string optionValue)
		{
			#if UNITY_EDITOR
			...
			// Set the value
			this.m_SelectedItem = optionValue;
		...
		public int GetOptionIndex(string optionValue)
		{
			...
			if (this.options.Count > 0 && !string.IsNullOrEmpty(optionValue))
		...
		protected void CreateList()
		{
			...
			// Check if the background sprite is set
		...
		protected void CreateSeparator(int index)
		{
			...
			le.preferredHeight = this.listSeparatorSprite.rect.height;
		}
